...
male_female_df<- filter(
  stats_df,
  contrast =="male_female" #only keeps rows where this is true
)
```
...
ggplot(
  ...
  # Now we are adding on a layer to specify what kind of plot we want. uses a + sign
geom_point()
```
...
ggplot(
  ...
  # Now we are adding on a layer to specify what kind of plot we want. uses a + sign
geom_point(alpha = 0.2) #alpha makes point more opaque
```
...
ggplot(
  ...
  labs(
    x = "log2 Fold Change Tumor/Normal",
    y = "-log10 p value",
    title = "Male vs Female Cerebellum",
    # Use the color keyword to label the color legend
    color = "Average expression",
    caption = "caption"
  )
