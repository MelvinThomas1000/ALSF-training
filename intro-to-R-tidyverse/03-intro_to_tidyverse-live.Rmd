...
stats_df |>
  ...
  select(neg_log10_p_value, everything()) #everything() adds everything else selects first something first
```
...
```{r summarize}
stats_df |>
  summarize(meanlfc = mean(log_fold_change),
           sdlfc = sd(log_fold_change))
```
...
stats_summary_df <- stats_df |>
  group_by(contrast) |> #group_by has to be first in order to summarize by group
  summarize(meanlfc = mean(log_fold_change),
           sdlfc = sd(log_fold_change))
```
...
readr::write_rds(stats_summary_df,
                 file.path("results", "stats_summary.RDS"))
```
