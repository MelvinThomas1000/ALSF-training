...
file.exists(file.path("data", "gene_results_GSE44971.tsv"))

stats_df <- read_tsv(file.path("data", "gene_results_GSE44971.tsv"))
```
...
file.exists(
  file.path("data", "cleaned_metadata_GSE44971.tsv")
)

metadata<- read_tsv(file.path("data", "cleaned_metadata_GSE44971.tsv")
)
```
...
log2(min(stats_df$avg_expression))
```
...
filter(stats_df, contrast == "astrocytoma_normal")
```
...
astrocytoma_normal_df<- dplyr::filter(
...
write_tsv(
  ...
  file.path("results2","astrocytoma_normal_contrast_results.tsv"
))
```
...
ggsave(
plot = densityplot,
filename = file.path("results2", "astrocytoma_normal_data_desity-plot.PNG"),
width = 8,
height = 6
)
